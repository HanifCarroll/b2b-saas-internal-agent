"""Low-level demo replacement stays atomic and explicit."""
...
def test_reset_clears_business_work_and_history(tmp_path):
    ...
    assert setup.scenario_id == "unregistered-destination"
...
def test_reset_is_rejected_while_an_operation_is_active_across_processes(tmp_path):
    path = tmp_path / "switchboard.db"
    runs = tmp_path / "workflows"
    reset_demo(database_path=path, runs_directory=runs, scenario_id="baseline")
...
def test_failed_replacement_preserves_existing_demo(tmp_path, monkeypatch):
...
def test_cli_reset_requires_confirmation_and_reports_active_scenario(
    ...
    path = tmp_path / "switchboard.db"
    runs = tmp_path / "workflows"
    monkeypatch.setattr(cli, "DATABASE_PATH", path)
    monkeypatch.setattr(cli, "RUNS_DIRECTORY", runs)
